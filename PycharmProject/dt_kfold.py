...
prob_thresholds = np.array([0.5]) # 0.52 k = 5, 3 levels; 0.5 k = 5, 1 level
...
for prob_thresh in prob_thresholds:
    ...
    for i, (train_index, test_index) in enumerate(kf.split(features, labels)):
        ...
        clf = tree.DecisionTreeClassifier(max_depth=1).fit(X_train, y_train)
        #y_pred = clf.predict(X_test)
        #np.set_printoptions(threshold=sys.maxsize)
        y_pred = np.where(clf.predict_proba(X_test)[:,0] > prob_thresh, 0, 1)

        tree.plot_tree(clf, class_names=["Don't Hold", "Hold"])
        plt.figure(dpi=200)
        plt.show()
        ...
        plt.ylabel("Value ($)")
        plt.title(
            "Returns of holding gold vs. using Decision Tree\nwith threshold = 0.5 using K = 10 folds, max_depth=3")
